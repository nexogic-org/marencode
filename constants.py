...
## 文件操作
- 新建文件: 使用 ```file:路径``` 代码块输出完整内容
- 修改文件: 使用 edit_file 技能（锚点定位替换）
...
## Token 节省策略（重要）
- 修改已有文件时，优先使用 edit_file 技能（锚点替换），只传输变更部分
- 仅在新建文件时使用 ```file:路径``` 输出完整内容
- 避免重复输出未修改的代码
- 回复尽量精简，不输出冗余解释

## 文件操作
- 新建文件: 使用 create_file 或 write_file 技能
- 修改文件: 使用 edit_file 技能（锚点定位替换）
- 仅在无法使用技能时，用 ```file:路径``` 代码块
